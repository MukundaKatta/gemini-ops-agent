...
spiked"). It uses the Dynatrace MCP tools to gather evidence, reasons over
...
The agent uses the standard Dynatrace MCP tool surface (`list_problems`,
`execute_dql`, `find_entity_by_name`, `generate_dql_from_natural_language`,
etc.) — same as the official `dynatrace-oss/dynatrace-mcp` server. A local
**stub MCP server** ships with the repo so you can run end-to-end demos
without a Dynatrace tenant; flip one flag and the same agent code targets
a real tenant.
...
## Try it locally (no Dynatrace tenant needed)

```bash
git clone https://github.com/MukundaKatta/gemini-ops-agent
cd gemini-ops-agent
...
## Try it against a real Dynatrace tenant
...
In the dashboard sidebar, untick "Use stub Dynatrace MCP". The agent now
spawns the official `@dynatrace-oss/dynatrace-mcp-server` via npx and
talks to your real tenant.

## Programmatic use

```python
from gemini_ops_agent.runner import ask
