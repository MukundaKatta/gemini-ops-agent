# Google Cloud Rapid Agent Hackathon — Dynatrace track submission
...
| Integrates a Partner's MCP server | Tool surface matches `dynatrace-oss/dynatrace-mcp`; stub for demos, real tenant via env var |
...
A Gemini agent built with Google's ADK that pulls Dynatrace problems and DQL
results via MCP, then reasons over them to give an SRE root-cause analysis.

## Description (paste into Devpost)
gemini-ops-agent treats every production symptom as an SRE ticket. You ask
"why is the checkout-api latency spiking?" and the agent works the case:
calls `list_problems` on the Dynatrace MCP server to see what Davis AI has
...
Kit (ADK) and runs Gemini 2.5 on Vertex AI. It talks to the Dynatrace MCP
server via the standard MCP protocol; a stub server ships in the repo so
reviewers can run end-to-end demos without a Dynatrace tenant, and the
...
agent-development-kit, mcp, dynatrace, dynatrace-mcp, streamlit,
...
- [x] Integrates Dynatrace MCP server (tool surface compatible)
