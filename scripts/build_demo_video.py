"""Build the gemini-ops-agent demo video end-to-end.
...
def base(img, d, title=None, eyebrow=None):
    ...
    d.text((48, H - 44), "gemini-ops-agent", font=font(22), fill=FG)
    d.text((W - 600, H - 44), "github.com/MukundaKatta/gemini-ops-agent", font=font(22), fill=FG_MUTED)
    if eyebrow:
...
def draw_title(img, d):
    ...
    d.rectangle([(0, H - 56), (W, H)], fill=PANEL)
    d.text((48, H - 44), "github.com/MukundaKatta/gemini-ops-agent", font=font(22), fill=FG_MUTED)
    d.text((W - 270, H - 44), "Apache 2.0", font=font(22), fill=FG_MUTED)
    d.text((96, 320), "gemini-ops-agent", font=font(130), fill=FG)
    d.rectangle([(96, 490), (340, 500)], fill=ACCENT)
    d.text((96, 540), "A Gemini agent that investigates", font=font(48), fill=FG_MUTED)
    d.text((96, 600), "production incidents using Dynatrace MCP.", font=font(48), fill=FG_MUTED)
    ...
    d.text((96, 850), "Dynatrace partner track.", font=font(32), fill=FG)
...
def draw_problem(img, d):
    ...
    rows = [
        ...
        "You open Dynatrace, hunt for the right query, scroll problems,",
    ...
    for line in rows:
        ...
        y += 76
    d.text((96, 800), "gemini-ops-agent does that conversation in under 10 seconds.", font=font(34, italic=True), fill=ACCENT_2)
...
def draw_architecture(img, d):
    ...
    boxes = [
        ...
        ("Dynatrace MCP", "list_problems, execute_dql, ...", ACCENT_2),
    ]
    x = cx
    ...
    d.text((96, 600), "MCP tool surface matches the official dynatrace-oss/dynatrace-mcp", font=font(32), fill=FG)
    d.text((96, 650), "server. Flip one flag and the agent targets a real Dynatrace tenant.", font=font(32), fill=FG)
...
def draw_question(img, d):
    ...
    d.text((96, 560), "Agent steps through the Dynatrace MCP tools:", font=font(32), fill=FG_MUTED)
    steps = [
        "1.  list_problems          →  Davis AI's open incidents",
...
def draw_code(img, d):
    ...
    code = (
        ...
        "from gemini_ops_agent.agent import _dynatrace_toolset\n"
        "\n"
        "agent = LlmAgent(\n"
        "    model='gemini-2.5-flash',\n"
        "    name='gemini_ops_agent',\n"
        "    instruction=SYSTEM_PROMPT,\n"
        "    tools=[_dynatrace_toolset(stub=True)],\n"
        ")"
    )
...
def draw_tests(img, d):
    ...
    d.text((96, 670), "gemini-ops-agent-1029931682737.us-central1.run.app", font=font(34, mono=True), fill=ACCENT_2)
    d.text((96, 800), "Repo: github.com/MukundaKatta/gemini-ops-agent",
           font=font(32, italic=True), fill=FG_MUTED)


def draw_close(img, d):
    d.rectangle([(0, 0), (W, H)], fill=BG)
    d.text((96, 200), "gemini-ops-agent", font=font(96), fill=FG)
    d.rectangle([(96, 320), (340, 330)], fill=ACCENT)
    d.text((96, 370), "github.com/MukundaKatta/gemini-ops-agent", font=font(40, mono=True), fill=ACCENT)
    d.text((96, 450), "gemini-ops-agent-1029931682737.us-central1.run.app", font=font(40, mono=True), fill=ACCENT_2)
    ...
    d.text((96, 680), "+ Dynatrace MCP server (stub for demo, real-tenant ready)", font=font(38), fill=FG_MUTED)
    d.text((96, 820), "Apache 2.0. Mukunda Katta, independent.", font=font(30, italic=True), fill=FG_MUTED)
    d.text((96, 870), "Submission for Google Cloud Rapid Agent Hackathon, Dynatrace track.", font=font(30, italic=True), fill=FG_MUTED)


SLIDES = [
    Slide("01_title",
          "Gemini ops agent. A Gemini agent that investigates production incidents using the Dynatrace MCP server, built on Google Cloud's Agent Development Kit.",
          draw_title),
    Slide("02_problem",
          "Picture being on call. You get a page. The checkout API's latency is spiking. You open Dynatrace, hunt for the right query, scroll problems, correlate with deployments, and twenty minutes later you find the root cause. Gemini ops agent does that conversation in under ten seconds.",
          draw_problem),
    Slide("03_architecture",
          "The architecture is three boxes. A user question goes into an A D K L L M agent powered by Gemini two point five on Vertex A I. The agent uses M C P toolset to call the Dynatrace M C P server. The server exposes the standard tools list problems, execute D Q L, find entity by name. With a stub server for demos, or the official server for real tenants. Every Gemini call is also traced through Gemini Lens for self observation.",
          draw_architecture),
    Slide("04_question",
    ...
    Slide("07_tests",
          ...
          draw_tests),
    Slide("08_close",
          "Gemini ops agent. Apache two point zero. Built for the Google Cloud Rapid Agent Hackathon, Dynatrace partner track. Thank you.",
          draw_close),
]
...
def main():
    outdir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path.home() / "gemini-ops-agent" / ".video-build"
