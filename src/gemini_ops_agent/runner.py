...
from gemini_ops_agent.agent import build_agent
...
def _observer():
    ...
    try:
        ...
        return None
    store = TraceStore(Path.home() / ".gemini-ops-agent" / "traces.jsonl")
    return GeminiObserver(store=store, default_tags={"agent": "gemini_ops_agent"})
