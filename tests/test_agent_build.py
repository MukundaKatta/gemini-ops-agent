from gemini_ops_agent.agent import build_agent, _ADK_AVAILABLE
...
def test_agent_constructs_with_default_model():
    ...
    assert agent.name == "gemini_ops_agent"
...
def test_agent_has_mcp_toolset():
    ...
    assert len(tools) >= 1, "agent should have at least the Dynatrace MCP toolset wired"
